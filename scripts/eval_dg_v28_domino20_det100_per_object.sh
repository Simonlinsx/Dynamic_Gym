...
RUN_NAME=${RUN_NAME:-dynamic_grasp_domino20_v28_2026-05-26_05-21-02}
TRAIN_GROUP=${TRAIN_GROUP:-dynamic_grasp_domino20}
TASK=${TASK:-SimToolRealDynamicGraspSim2RealPointCloudObjectVelFastRewardV28Domino20PointNet}
...
echo "Deterministic DOMINO20 eval"
echo "  task:       $TASK"
...
python -m isaacgymenvs.train \
  ...
  task="$TASK" \
  experiment=00_dg_v28_domino20_det100_per_object_eval \
  test=True \
  checkpoint="$SNAPSHOT" \
  headless=True \
