#!/usr/bin/env bash
set -euo pipefail

cd /data1/linsixu/simtoolreal
mkdir -p train_logs
exec > >(tee /data1/linsixu/simtoolreal/train_logs/dg_v33_franka_brainco_revo2_affordance_domino20_pointnet.log) 2>&1

source /data1/linsixu/miniconda3/etc/profile.d/conda.sh
conda activate simtoolreal

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-6}"

if [ ! -f assets/generated/franka_brainco_revo2_right/franka_brainco_revo2_right.urdf ]; then
  bash scripts/prepare_franka_brainco_revo2_asset.sh
fi

python isaacgymenvs/launch_training.py \
  --task SimToolRealDynamicGraspV33FrankaBrainCoRevo2AffordanceDomino20PointNet \
  --custom-experiment-name dynamic_grasp_franka_brainco_revo2_affordance_v33 \
  --num-envs 12288 \
  --wandb-entity simonlsx \
  --wandb-project simtoolreal \
  --wandb-group dynamic_grasp_franka_brainco_revo2 \
  --capture-video True \
  --capture-video-freq 3000 \
  --capture-video-len 240
