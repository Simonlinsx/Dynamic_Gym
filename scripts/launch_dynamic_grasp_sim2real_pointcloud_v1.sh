...
python isaacgymenvs/launch_training.py \
  --task SimToolRealDynamicGraspSim2RealPointCloud \
  --custom-experiment-name "${EXPERIMENT_NAME:-dynamic_grasp_sim2real_pointcloud_v1}" \
  --seed "${SEED:-0}" \
  --num-envs "${NUM_ENVS:-8192}" \
  --num-blocks "${NUM_BLOCKS:-4}" \
  --wandb-entity "${WANDB_ENTITY:-simonlsx}" \
  --wandb-project "${WANDB_PROJECT:-simtoolreal}" \
  --wandb-group "${WANDB_GROUP:-dynamic_grasp_sim2real_pointcloud}" \
  "${wandb_activate_args[@]}" \
  --capture-video True \
