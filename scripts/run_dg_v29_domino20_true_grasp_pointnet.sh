...
mkdir -p train_logs
exec > >(tee /data1/linsixu/simtoolreal/train_logs/dg_v29_domino20_true_grasp_pointnet.log) 2>&1
...
python isaacgymenvs/launch_training.py \
  --task SimToolRealDynamicGraspSim2RealPointCloudObjectVelFastRewardV29Domino20PointNet \
  --custom-experiment-name dynamic_grasp_domino20_true_grasp_v29 \
  --num-envs 12288 \
