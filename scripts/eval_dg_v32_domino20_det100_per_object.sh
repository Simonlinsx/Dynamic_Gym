...
RUN_NAME=${RUN_NAME:-dynamic_grasp_domino20_affordance_v32_YYYY-MM-DD_HH-MM-SS}
TRAIN_GROUP=${TRAIN_GROUP:-dynamic_grasp_domino20_affordance}
TASK=${TASK:-SimToolRealDynamicGraspSim2RealPointCloudObjectVelFastRewardV32AffordanceDomino20PointNet}
...
EVAL_NAME=${EVAL_NAME:-${RUN_NAME}_det100_per_object_${EVAL_TIMESTAMP}}
EVAL_DIR=${EVAL_DIR:-$REPO_DIR/eval_videos/$EVAL_NAME}
EVAL_NUM_ENVS=${EVAL_NUM_ENVS:-100}
EVAL_GAMES_NUM=${EVAL_GAMES_NUM:-100000}
...
echo "Deterministic DOMINO20 affordance eval"
echo "  task:       $TASK"
...
echo "  protocol:   20 objects x 5 envs, first completed episode per env"

python -m isaacgymenvs.train \
  ++task.env.useSparseReward=False \
  task="$TASK" \
  experiment=00_dg_v32_domino20_det100_per_object_eval \
  test=True \
  checkpoint="$SNAPSHOT" \
  headless=True \
  ...
  +task.env.evalStopAfterEachEnvOnce=True \
